...
public class InternetHelper {
    ...
    public static String sendHttpRequest(String method, String urlString, ArrayList<KeyValue> data) throws Exception
    {
        method = method.toUpperCase();
        if(method.equals(InternetHelper.REQUEST_METHOD_GET))
        {
            data = InternetHelper.urlEncodeDataValues(data);
            urlString = (urlString + "?" + KeyValue.makeURIFormat(data));
        }
    ...
    {
        ArrayList<KeyValue> encodedData = new ArrayList<>();
    ...
    public static String uploadFiles(String urlString, FormData data) throws Exception {
        ArrayList<KeyValue> allFileData =
                data.getData(FormData.CONTENT_TYPE_MULTIPART_FILE);
        ArrayList<KeyValue> allPlainData =
                data.getData(FormData.CONTENT_TYPE_PLAIN_TEXT);
        String crlf = "\r\n";
        String twoHyphens = "--";
        ...
        if(responseCode != HttpURLConnection.HTTP_OK)
        {
            throw new Exception("Failed (Response Code: " + responseCode +
                    ") to establish connection with host " +
                    connection.getURL().toString());
        }
        String response = InternetHelper.retrieveResponseString(connection);
        return response;
